...
### Аппрув групп (кто может добавлять ботов)

Чтобы ботами не пользовались в случайных чатах, у @PastilaTaskBot и GPT-бота есть гейт:

- Бота добавили в **новую** группу → он там молчит, а админам (`ADMIN_USER_IDS`) в личку летит запрос с кнопками **✅ Разрешить / ❌ Запретить** (и то же при первом сообщении из ещё-не-одобренной группы).
- **Разрешить** → группа попадает в список одобренных, бот здоровается и работает.
- **Запретить** → бот выходит из группы.
- Хранилище одобренных групп: у **TaskBot** — ячейка `llm_config!A2` (переживает редеплой); у **GPT-бота** — файл `approved-chats.json` (эфемерная ФС Render, сбрасывается при передеплое — тогда одобрить заново).
- Основная рабочая группа (`GROUP_CHAT_ID`) добавляется в одобренные автоматически, чтобы не отвалиться после обновления. Выключается флагом `GROUP_APPROVAL=false`.
- У **бриджа** доступ управляется отдельно — скиллом `/telegram:access` в терминале (не из чата).
