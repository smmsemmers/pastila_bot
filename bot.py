...
async def _maybe_handle_nl(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if BRIDGE_PRIMARY:
        return  # на вопросы отвечает бридж (главный); task-бот — только команды/расписания
    msg = update.message
...
async def _maybe_check_insight(chat_id: int, msg_id: int, msg_text: str, bot) -> None:
    ...
    if BRIDGE_PRIMARY:
        return  # инсайты не нужны, когда главный — бридж
    if not llm.OPENROUTER_API_KEY:
        return
...
FILE_ACTIONS = [
    ...
    ("plan",     "🗺 План действий",  "Составь конкретный план действий по этому материалу: шаги по порядку, "
                                       "что делать. Пронумеруй, по возможности — кто и срок."),
    ("strategy", "🧭 Стратегия",      "Определи стратегию: куда это ведёт, какой должна быть линия действий, "
                                       "чего добиваемся и как. Дай обоснованный взгляд, а не пересказ."),
    ("swot",     "⚖️ Сильные/слабые", "Определи сильные и слабые стороны (и возможности/угрозы, если видно). "
                                       "Кратко, по пунктам."),
    ("priority", "🎯 Приоритеты",     "Что самое важное и срочное прямо сейчас. Топ-3–5 по приоритету, с обоснованием."),
    ...
    ("advice",   "💡 Советы",         "Дай конкретные рекомендации: что делать с этим, следующие шаги."),
]
...
async def on_file_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await wait.edit_text(text[:3850] + f"\n\n🧠 {llm.key_label(fa_key)}")
...
async def on_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await _handle_session_file(update, context):
        return
    if BRIDGE_PRIMARY:
        return  # файлы разбирает бридж (главный); task-бот в них не лезет
...
START_TEXT = (
    ...
    "<b>⚙️ Ещё команды</b>\n"
    "<code>/dash</code> сводка · <code>/deep</code> глубокий анализ (можно "
    "<code>/deep тема</code>) · <code>/strategy</code> стратегия\n"
    "<code>/session</code> · <code>/kb</code> · <code>/find</code> — база знаний · "
    "<code>/notion</code> синк\n"
    "<code>/model</code> · <code>/fable</code> · <code>/trim</code> · "
    "<code>/tokens</code> — модели и контекст · <code>/welcome</code> баннер\n\n"
    "<b>🎙️ Голос</b>\n"
...
WELCOME_CAPTION = (
    ...
    "🤖 <b>TaskBot</b> — ✓ задачи, дедлайны, напоминания (я, всегда онлайн).\n"
    "💬 <b>GPT Remote</b> — 🔍 исследователь: поиск в интернете, deep research, OCR.\n\n"
    "Ниже — я, <b>TaskBot</b>: чтобы поручения не терялись, каждая задача оформлена "
    "и записана — видно <b>кто, что и к какому сроку</b>.\n\n"
    "<b>📋 Команды</b>\n"
...
PIN_MAIN_TEXT = (
    "🍬 <b>Pastila OS — рабочий штаб в Telegram</b>\n\n"
    "Здесь три ИИ-бота помогают с задачами, файлами и аналитикой бизнеса.\n"
    "Нажми плитку ниже — расскажу, что как устроено, зачем и куда всё идёт.\n\n"
    "👥 <b>Глеб</b> @foxruso · <b>Лена</b> @elenaisanewleet"
)
...
PIN_SECTIONS = {
    ...
    "files": (
        "📥 Что кидать",
        "<b>📥 Кидай файлы — бот разберёт</b>\n\n"
        "Документ, таблицу Excel, PDF, фото, голосовое → бот читает целиком и даёт "
        "разбор + рекомендации, а под ним <b>кнопки</b> действий.\n\n"
        "Несколько файлов альбомом → разбирает вместе.\n"
        "Голос → расшифровка без «ммм», по сути.\n"
        "Всё складывается в архив (Google Sheets) и базу знаний.",
    ),
    "analysis": (
        "🧭 Анализ",
        "<b>🧭 10 кнопок под каждым файлом</b>\n\n"
        "📋 Разбор · ✂️ Саммари · 🗺 План действий · 🧭 Стратегия · ⚖️ Сильные/слабые · "
        "🎯 Приоритеты · ✅ Задачи · ⚠️ Риски · 📊 Цифры · 💡 Советы\n\n"
        "Жмёшь — бот делает именно это по файлу (Claude Opus). "
        "Определяет стратегию, слабые/сильные места, приоритеты и план по твоим данным.",
    ),
    "tasks": (
        "✅ Задачи",
        "<b>✅ Задачи — словами или командой</b>\n\n"
        "Просто напиши: «поставь встречу на 15», «напомни завтра про поставщика» — "
        "бот сам заведёт задачу.\n\n"
        "Или команды: <code>/new</code> (по шагам), <code>/list</code> (открытые), "
        "<code>/digest</code> (дедлайны сегодня), <code>/alerts</code> (на завтра).",
    ),
    "research": (
        "🔎 Исследование",
        "<b>🔎 Поиск и исследование (GPT-бот)</b>\n\n"
        "<code>/research вопрос</code> — быстрый веб-поиск со ссылками.\n"
        "<code>/agent задача</code> — глубокое исследование (отчёт + десятки источников).\n"
        "<code>/model</code> — выбрать модель (экономия токенов).\n\n"
        "Пример: <i>/agent поставщики упаковки в ЕС с ценами</i>.",
    ),
    "where": (
        "🗂 Куда идёт",
        "<b>🗂 Где всё хранится</b>\n\n"
        "📊 <b>Google Sheets</b> — задачи + лист «Контент» (архив всего присланного: "
        "дата, тип, тема, актуальность). Сортируется по любому столбцу.\n\n"
        "🧠 <b>База знаний</b> — ищется через <code>/find</code>, важные мысли "
        "сохраняются сами (⭐).",
    ),
    "why": (
        "❓ Зачем",
        "<b>❓ Зачем это всё</b>\n\n"
        "Чтобы задачи и информация не терялись в переписке, а бизнес был под рукой: "
        "кинул файл — получил понятный разбор и план; сказал словами — завелась задача; "
        "спросил — бот нашёл в интернете.\n\n"
        "Цель — оптимизация и автоматизация процессов SEMERS.",
    ),
    "start": (
        "🚀 Как начать",
        "<b>🚀 С чего начать</b>\n\n"
        "1. Кинь любой файл → жми кнопки разбора.\n"
        "2. Напиши «поставь задачу …» — заведётся.\n"
        "3. Нужен поиск → <code>/research</code> в GPT-боте.\n"
        "4. Нужен код/деплой/большой экспорт → запусти бридж "
        "(<code>start-code-bridge.command</code>).",
    ),
}
...
async def cmd_deep(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/deep [тема] — двухэтапный анализ: Perplexity ищет в интернете, Opus синтезирует
    с внутренними данными. Если после /deep задать текст — он становится темой
    исследования; без текста используется дефолтный пресет (рынок белёвской пастилы)."""
    msg = update.effective_message
    scope = " ".join(context.args).strip() if context.args else ""
    wait = await msg.reply_text(
        (f"🔬 Запускаю глубокий анализ по теме:\n«{scope}»\n"
         if scope else "🔬 Запускаю глубокий анализ…\n")
    ...
    try:
        ...
        await wait.edit_text(
            (f"🌐 Этап 1/2 — Perplexity исследует тему «{scope}»…\n(60–90 сек)"
             if scope else
             "🌐 Этап 1/2 — Perplexity исследует рынок и конкурентов…\n(60–90 сек)")
        )
        if scope:
            web_user_prompt = (
                f"Исследуй тему: {scope}\n"
                "Собери актуальную картину (2024–2026): ключевые факты и цифры, "
                "игроки/конкуренты и их позиционирование, тренды, каналы, "
                "что ценит аудитория, риски и незанятые возможности.\n\n"
                "Только конкретные факты, цифры, названия, источники. Без воды."
            )
        else:
            web_user_prompt = (
...
async def _set_commands(app):
    ...
    await app.bot.set_my_commands(
        [
            ...
            BotCommand("plan", "🍭 План для Лены и Глеба"),
            BotCommand("q", "🍬 Быстро сохранить идею"),
            BotCommand("dash", "🍡 Сводка: задачи и база знаний"),
            BotCommand("post", "🍭 Черновик поста в соцсети"),
            BotCommand("recurring", "🍬 Повторяющиеся задачи"),
            BotCommand("remind_when", "🍭 Триггерное напоминание"),
            BotCommand("purge", "🍫 Удалить все задачи (с подтверждением)"),
            BotCommand("edit", "🍬 Изменить поле задачи"),
            BotCommand("tokens", "🍡 Токены и стоимость вызовов"),
            BotCommand("deep", "🍎 Глубокий анализ (веб → синтез)"),
            BotCommand("strategy", "🍫 Стратегический совет"),
            BotCommand("pin", "🍡 Описание группы с навигацией"),
            BotCommand("ai", "🍬 Проверить связь OpenAI/OpenRouter"),
            BotCommand("session", "🍡 Добавить сессию в базу знаний"),
            BotCommand("find", "🍎 Найти в базе знаний"),
            BotCommand("notion", "🍡 Синхронизировать Notion → база"),
            BotCommand("kb", "🍡 Показать базу знаний"),
            BotCommand("welcome", "🍬 Привет и что умеет бот"),
            BotCommand("id", "🍭 ID этого чата"),
            BotCommand("cancel", "🍫 Отменить создание задачи"),
            BotCommand("start", "🍬 Запуск и что умеет бот"),
