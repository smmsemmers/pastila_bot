    ...
    if (/^\/status/i.test(rawText)) {
      ...
      await sendLong(
        ...
        [
          ...
          `user_id: ${msg.from?.id}`,
          `provider_mode: ${USE_OPENROUTER ? "OpenRouter (авто-роутинг)" : "OpenAI (одна модель)"}`,
          `classifier: ${USE_OPENROUTER ? CLASSIFIER_MODEL : "—"}`,
          "route_map:",
          ...routeLines,
          `research_model (/research, /agent): ${pickModel(chatId).label}`,
          `deep_research_model (/agent): ${DEEP_RESEARCH_MODEL}`,
          `fallback_model (OPENAI_MODEL): ${FALLBACK_MODEL}`,
